...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnapEditor : Editor
    {
        ...
        private void HeaderInformation()
        {
            ...
            GUILayout.Label("Version: 1.0.1", new GUIStyle() { fontSize = 14, alignment = TextAnchor.MiddleCenter });
        ...
        {
            using (new EditorGUI.DisabledScope(numberOfPanels < 2))
            {
                ...
                if (simpleScrollSnap.infinitelyScroll && numberOfPanels >= 2)
                {
                    EditorGUI.indentLevel++;
            ...
            if (numberOfPanels < 2)
            {
                simpleScrollSnap.infinitelyScroll = false;
                simpleScrollSnap.infiniteScrollingEndSpacing = 0f;
                EditorGUILayout.HelpBox("Two or more panels are required in order for panels to be infinitely scrolled.", MessageType.Info);
            }
        }
        ...
        private void ExecutionEvents()
        {
            EditorStyles.foldout.fontStyle = FontStyle.Bold;
            showEvents = EditorGUILayout.Foldout(showEvents, "Execution Events", true);
            EditorStyles.foldout.fontStyle = FontStyle.Normal;
